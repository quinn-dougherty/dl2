...
class DL2_Oracle:
    ...
    def attack(self, x_batch, y_batch, domains, num_restarts, num_iters):
        ...
        for retry in range(num_restarts):
            z_batch = np.concatenate(
                [np.expand_dims(domains[i].sample(), axis=0) for i in range(n_batch)],
                axis=0,
            )

            for it in range(num_iters):
                avg_neg_loss, avg_pos_loss, sat, z_inputs = self.loss(
                    self.net, x_batch, y_batch, z_batch, self.use_cuda
                )
    ...
    def general_attack(
        self, x_batches, y_batches, domains, num_restarts, num_iters, args
    ):
        ...
        for retry in range(num_restarts):
            z_batches = [
                np.concatenate(
                    [
                        np.expand_dims(domains[j][i].sample(), axis=0)
                        for i in range(n_batch)
                    ],
                    axis=0,
                )
            ...
            assert (
                z_batches[0].shape[0] == n_batch
            ), "Batch size in x and z doesn not match"

            for it in range(num_iters):
                neg_losses, pos_losses, sat, z_inputs = self.constraint.loss(
    ...
    def evaluate(self, x_batches, y_batches, z_batches, args):
        neg_losses, pos_losses, sat, _ = self.constraint.loss(
            x_batches, y_batches, z_batches, args
        )
