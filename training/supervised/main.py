...
def train(args, oracle, net, device, train_loader, optimizer, epoch, pca=None):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        assert (
            n_batch % oracle.constraint.n_tvars == 0
        ), "Batch size must be divisible by number of train variables!"
        ...
        if oracle.constraint.n_gvars > 0:
            ...
            _, dl2_batch_loss, constr_acc = oracle.evaluate(
                x_batches, y_batches, z_batches, args
            )
        else:
            _, dl2_batch_loss, constr_acc = oracle.evaluate(
                x_batches, y_batches, None, args
            )
        ...
        if batch_idx % args.print_freq == 0:
            print(
                "[%d] Train acc: %.4f, Constr acc: %.4f, CE loss: %.3lf, DL2 loss: %.3lf"
                % (
                    batch_idx,
                    x_correct.item(),
                    constr_acc.item(),
...
parser.add_argument(
    "--batch-size", type=int, default=128, help="Number of samples in a batch."
)
parser.add_argument(
    "--num-iters", type=int, default=50, help="Number of oracle iterations."
)
parser.add_argument(
    "--num-epochs", type=int, default=300, help="Number of epochs to train for."
)
parser.add_argument("--dl2-weight", type=float, default=0.0, help="Weight of DL2 loss.")
parser.add_argument(
    "--dataset", type=str, default="mnist", help="Dataset on which to train."
)
parser.add_argument(
    "--pretrained", action="store_true", help="Whether to use pretrained model."
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="How many epochs to wait before training with constraints.",
)
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help="the constraint to train with: LipschitzT(L), LipschitzG(eps, L), RobustnessT(eps1, eps2), RobustnessG(eps, delta), CSimiliarityT(), CSimilarityG(), LineSegmentG()",
)
parser.add_argument("--embed-dim", type=int, default=40, help="embed dim")
parser.add_argument(
    "--network-output",
    type=str,
    choices=["logits", "prob", "logprob"],
    default="logits",
    help="Wether to treat the output of the network as logits, probabilities or log(probabilities) in the constraints.",
)
...
if args.dataset == "mnist":
    ...
    train_loader = torch.utils.data.DataLoader(
        mnist_data, shuffle=True, batch_size=args.batch_size, **kwargs
    )
    ...
    train_loader = torch.utils.data.DataLoader(
        fashion_data, shuffle=True, batch_size=args.batch_size, **kwargs
    )
    test_loader = torch.utils.data.DataLoader(
    ...
    train_loader = torch.utils.data.DataLoader(
        cifar_data, shuffle=True, batch_size=args.batch_size, **kwargs
    )
    test_loader = torch.utils.data.DataLoader(
        datasets.CIFAR10(
            "../../data/cifar10", train=False, download=True, transform=transform_test
        ),
        batch_size=256,
        shuffle=True,
        **kwargs
...
def RobustnessT(eps1, eps2):
    return lambda model, use_cuda, network_output: RobustnessDatasetConstraint(
        model, eps1, eps2, use_cuda=use_cuda, network_output=network_output
    )


def RobustnessG(eps, delta):
    return lambda model, use_cuda, network_output: RobustnessConstraint(
        model, eps, delta, use_cuda, network_output=network_output
    )


def LipschitzT(L):
    return lambda model, use_cuda, network_output: LipschitzDatasetConstraint(
        model, L, use_cuda, network_output=network_output
    )


def LipschitzG(eps, L):
    return lambda model, use_cuda, network_output: LipschitzConstraint(
        model, eps=eps, l=L, use_cuda=use_cuda, network_output=network_output
    )


def CSimilarityT(delta):
    return lambda model, use_cuda, network_output: CifarDatasetConstraint(
        model, delta, use_cuda, network_output=network_output
    )


def CSimilarityG(eps, delta):
    return lambda model, use_cuda, network_output: CifarConstraint(
        model, eps, delta, use_cuda, network_output=network_output
    )


def SegmentG(eps, delta):
    return lambda model, use_cuda, network_output: PairLineRobustnessConstraint(
        model, eps, delta, use_cuda, network_output=network_output
    )
...
oracle = DL2_Oracle(
    learning_rate=0.01, net=model, constraint=constraint, use_cuda=use_cuda
)

opt_type = "T" if constraint.n_gvars == 0 else "G"
report_dir = os.path.dirname(
    os.path.join(
        args.report_dir, "%s/%s/%s" % (opt_type, args.dataset, constraint.name)
    )
)
...
report_file = os.path.join(
    report_dir, "report_%s_%s_%d.json" % (constraint.name, exptype, tstamp)
)
data_dict = {
...
for epoch in range(1, args.num_epochs + 1):
    avg_train_acc, avg_constr_acc, avg_dl2_loss, avg_ce_loss, epoch_time = train(
        args, oracle, model, device, train_loader, optimizer, epoch, pca
    )
