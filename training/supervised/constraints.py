...
class LipschitzConstraint(Constraint):
    ...
    def get_condition(self, z_inp, z_out, x_batches, y_batches):
        ...
        return dl2.LEQ(
            torch.norm(z_out[0] - z_out[1], p=2, dim=1),
...
class PairLineRobustnessConstraint(Constraint):
    ...
    def __init__(self, net, eps, p_limit, use_cuda=True, network_output="logits"):
        ...
        self.network_output = network_output
        print(
            "Ignoring network_output argument, using logprob such that loss becomes cross-entropy"
        )
        self.net = net
        self.eps = eps
    ...
    def get_condition(self, z_inp, z_out, x_batches, y_batches):
        ...
        pre = dl2.And(
            [
                ...
                dl2.LEQ(
                    torch.norm(
                        (x_batches[0] - x_batches[1]).view((n_batch, -1)), dim=1
                    ),
                    self.eps,
                ),
            ]
...
class LipschitzDatasetConstraint(Constraint):
    ...
    def get_condition(self, z_inp, z_out, x_batches, y_batches):
        ...
        return dl2.LEQ(
            torch.norm(x_out1 - x_out2, p=2, dim=1),
            self.l
            * torch.norm((x_batches[0] - x_batches[1]).view((n_batch, -1)), p=2, dim=1),
        )
...
class RobustnessDatasetConstraint(Constraint):
    ...
    def get_condition(self, z_inp, z_out, x_batches, y_batches):
        ...
        close_x = dl2.LT(
            torch.norm((x_batches[0] - x_batches[1]).view((n_batch, -1)), dim=1),
            self.eps1,
        )
