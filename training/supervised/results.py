...
history = [
    [None for j in range(len(datasets) * 2)] for i in range(len(constraints) * 2)
]

parser = argparse.ArgumentParser(description="Supervised DL2 Training Result printer")
parser.add_argument(
    "--folder", type=str, default="./results", help="Folder where results are stored."
)
parser.add_argument(
    "--plot-dir", type=str, default="./plots", help="Folder where plots will be stored."
)
parser.add_argument(
    "--strategy",
    "-s",
    ...
    default="prodMaxOfLast25",
    help="How to pick the values",
)
...
def params_to_constraint(data, ds, postfix):
    ...
    if "L" in params:
        name = "Lipschitz"
        flag = (
            (params["L"] == 1.0 and postfix == "T")
            or (params["L"] == 0.1 and postfix == "G" and ds != "fashion")
            or (
                params["L"] == 0.3
                and postfix == "G"
                and ds == "fashion"
                and (data["dl2_weight"] == 0.2 or data["dl2_weight"] == 0)
            )
            or (params["L"] == 1.0 and ds == "cifar10")
        )
    ...
    for j, ds in enumerate(datasets):
        if table[2 * i][2 * j] is not None:
            ...
            if ds == "fashion" and c == "SegmentG":
                x = 208
                ax.text(
                    x, 0.20, "C-Acc, Baseline", fontsize=fontsize, color=l1.get_color()
                )
                ax.text(x, 0.50, "C-Acc, DL2", fontsize=fontsize, color=l2.get_color())
                ax.text(
                    x, 0.88, "P-Acc, Baseline", fontsize=fontsize, color=l1.get_color()
                )
            ...
            if ds == "fashion" and c == "LipschitzT":
                x = 208
                ax.text(
                    x, 0.05, "C-Acc, Baseline", fontsize=fontsize, color=l1.get_color()
                )
                ax.text(x, 0.985, "C-Acc, DL2", fontsize=fontsize, color=l2.get_color())
                ax.text(
                    x, 0.920, "P-Acc, Baseline", fontsize=fontsize, color=l1.get_color()
                )
