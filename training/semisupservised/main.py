...
parser.add_argument(
    "--dataset", default="cifar100", type=str, help="dataset = [cifar10/cifar100]"
)
...
parser.add_argument(
    "--testOnly", action="store_true", help="Test mode with the saved model"
)
parser.add_argument(
    "--constraint",
    type=str,
...
parser.add_argument(
    "--c-eps", default=0.05, type=float, help="Number of labeled examples (per class!)."
)
parser.add_argument(
...
if args.testOnly:
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        conf_mat += confusion_matrix(
            targets.data.cpu().numpy(), predicted.cpu().numpy(), labels=np.arange(100)
        )
...
def train(epoch):
    ...
    for batch_idx, (lab, ulab) in enumerate(zip(tl, trainloader_unlab)):
        ...
        if args.constraint == "DL2":
            ...
            loss = (
                ce_loss
                + (args.constraint_weight * args.increase_constraint_weight**epoch)
                * dl2_loss
            )
        else:
            loss = ce_loss
        ...
        sys.stdout.write("\r")
        sys.stdout.write(
            "| Epoch [%3d/%3d] Iter[%3d/%3d]\t\tCE Loss: %.4f, Constraint Loss: %.4f Acc@1: %.3f%%"
            % (
                epoch,
                num_epochs,
                batch_idx + 1,
                (len(train_lab_idx) // batch_size) + 1,
                loss,
                constraint_loss,
