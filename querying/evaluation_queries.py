...
query_settings["FASHION_MNIST"]["class_not_var"] = query_settings["FASHION_MNIST"][
    "class"
]
...
query_settings["IMAGENET"]["var_to_class_var"] = [
    "640",
    "1",
    "338",
    "550",
    ...
    for j, query_file in enumerate(query_files):
        ...
        original_keys = list(
            set(
                [
                    k.replace("<<", "").replace(">>", "")
                    for k in re.findall("<<[a-zA-Z\_]+[0-9]*>>", query)
                ]
            )
        )
        ...
        for d in datasets:
            if d in query_settings:
                ...
                for vals in val_sets:
                    ...
                    if (
                        "class_var" in original_keys
                        and "class_not_var" in original_keys
                        and vals[keys.index("class_not_var")] == class_var
                    ):
