...
parser.add(
    ...
    default="*.tdql",
    required=False,
    help="pattern to glob for tdql files in ./evaluation_queries",
)
parser.add(
    "--dataset",
    choices=["MNIST", "FASHION_MNIST", "CIFAR", "GTSRB", "IMAGENT"],
    default=["MNIST", "FASHION_MNIST", "CIFAR", "GTSRB", "IMAGENT"],
    nargs="+",
...
for dataset in args.dataset:
    ...
    for query_name in queries[dataset]:
        ...
        print(
            f"{query_name}\t{solved_queries}/{len(qs)}\t{time_all:.2f}\t{time_solved:.2f}"
        )
